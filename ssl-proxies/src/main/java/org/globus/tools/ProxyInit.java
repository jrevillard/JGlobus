...
public abstract class ProxyInit {
	...
	public void verify() throws Exception {
		...
		if (proxyCertInfo != null) {
			...
			handlers.put(oid, new ProxyPolicyHandler() {
				public void validate(ProxyCertInfo proxyCertInfo, CertPath certPath, int index)
		...
		X509ProxyCertPathParameters parameters = new X509ProxyCertPathParameters(keyStore, crlStore, sigPolStore,
				false, handlers);
	...
	public void createProxy(String cert, String key, boolean verify, boolean globusStyle, String proxyFile) {
		...
		if (!quiet) {
			System.out.println("Your proxy is valid until " + proxy.getCertificateChain()[0].getNotAfter());
		}
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equalsIgnoreCase("-hours")) {
			...
			} else if (args[i].equalsIgnoreCase("-pl") || args[i].equalsIgnoreCase("-policy-language")) {
				if (i + 1 >= args.length) {
				...
				if (delimiterIndex == -1) {
					...
					String minutes = validity.substring(delimiterIndex + 1, validity.length());
		...
		if (proxyType == GSIConstants.CertificateType.UNDEFINED) {
			if (oid == 1) {
				proxyType = (limited) ? GSIConstants.CertificateType.GSI_3_LIMITED_PROXY
						: GSIConstants.CertificateType.GSI_3_IMPERSONATION_PROXY;
			} else if (oid == 2) {
				proxyType = (limited) ? GSIConstants.CertificateType.GSI_4_LIMITED_PROXY
						: GSIConstants.CertificateType.GSI_4_IMPERSONATION_PROXY;
			} else {
				proxyType = (limited) ? GSIConstants.CertificateType.GSI_2_LIMITED_PROXY
...
class DefaultProxyInit extends ProxyInit {
	...
	public void loadKey(String arg) {
		try {
			PrivateKey key = null;
			try {
				key = CertificateLoadUtil.loadPrivateKey(arg, null);
			} catch (PasswordException e) {
				String prompt = (quiet) ? "Enter GRID pass phrase: " : "Enter GRID pass phrase for this identity: ";
